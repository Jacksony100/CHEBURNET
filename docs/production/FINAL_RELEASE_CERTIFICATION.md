...
База: `78cc980a6d0d68f669cf98eff3799c1cf5fa02bd` (тег `v1.0.0-rc.3`).
Выпущенный кандидат: тег `v1.0.0-rc.4`.

Этот документ фиксирует состояние **кандидата** `1.0.0-rc.4` с выполненной
подготовкой к стабильному выпуску. Стабильный тег `v1.0.0` ещё не создан:
`cmake/Version.cmake` объявляет `rc.4`, и это верно — переключение на
стабильную версию выполняется одной правкой после закрытия блокеров ниже.
Попытка создать стабильный тег при RC-источнике отвергается шлюзом:
`tag version 1.0.0 does not match source semantic version 1.0.0-rc.4`.

## Launcher

| Поле | Значение |
|---|---|
| Semantic version | `1.0.0-rc.4` |
| Release channel | `prerelease` |
| PE FileVersion / ProductVersion | `1.0.0.4` |
| Authenticode | **не подписан** (сертификат в этом окружении отсутствует; для RC это разрешено политикой) |
| SHA-256 (локальная сборка) | `0d0c4b71aff0ea9948dea1d4d05a0a5416fda22882130e3723e0abd200546d81` |
| SHA-256 (опубликованный) | см. `CHEBURNET.exe.sha256` в активах выпуска `v1.0.0-rc.4` |
...
| Package SHA-256 (локальная сборка) | `e503705cd8a80eaa3e54e2f08d914f52462bcdc49d54bf79b7bd4701cb76449b` |
| Package size | 3 403 062 байта, 27 файлов |
...
### B4 — CodeQL и процесс static-analysis — **ЗАКРЫТО**

Задания `msvc-analyze` и `codeql` выполнены на GitHub для `main` и завершились
успешно, находок нет. Локальный эквивалент (MSVC `/analyze` с `/WX`) также чист.

### B5 — автоматизация upstream проверена частично

- **Серьёзность:** низкая, не блокирующая.
- **Выполнено:** процесс запущен вручную на `main` и завершился успешно.
  Задание `detect` отработало и выдало `UPSTREAM_REPORT: status=current
  current=1.10.2 latest=1.10.2`; задания `validate` и `task` корректно
  пропущены, поскольку создавать задачу не о чем.
- **Осталось:** ветки `validate`/`task` исполняются только при выпуске новее
  1.10.2. Разбор дельты, контракт отчёта и политика процесса покрыты тестом
  `upstream_automation` без сети.
- **Точное следующее действие:** после появления следующего выпуска Flowseal
  убедиться, что создаётся ровно одна задача и повторный запуск её обновляет,
  а не дублирует.

### B6 — запланированная проверка upstream падала еженедельно (исправлено)

- **Серьёзность:** средняя, обнаружена уже после слияния.
- **Причина:** в прежнем `upstream-check.yml` был однострочный `run:` с
  кириллицей в шаге `shell: powershell`. Раннер записывает блок во временный
  `.ps1` без BOM, а Windows PowerShell 5.1 читает его как ANSI, поэтому шаг
  падал с `CommandNotFoundException`. Процесс падал каждую неделю минимум с
  17 августа: доступность 1.10.2 обнаруживалась, но сигнал не доходил.
- **Исправление:** шаг удалён при переработке процесса; добавлен шлюз
  `workflow_encoding`, запрещающий не-ASCII во встроенном PowerShell всех
  процессов, включая однострочные `run:`. Подтверждено успешным ручным
  запуском на `main`.
